...
client.veri = require("./passenger/passenger-veri.json");
client.sistem = require("./passenger/passenger-ayar.json");
client.modüller = {}; 
client.altbaslik = client.veri.Tag + " " + client.veri.sunucuUfakIsim + " Developer by Passenger"
const qDb = require("quick.db");
const db = new qDb.table("ayarlar");
const cezaDb = new qDb.table("aCezalar");
//const passengerhook = new WebhookClient(client.veri.hosgeldinSistemi.webhookID, client.veri.hosgeldinSistemi.webhookTOKEN);
fs.readdir("./passenger/Komutlar", (err, files) => {
    if(err) return console.error(err);
    files = files.filter(file => file.endsWith(".js"));
    console.log(`[ ${files.length} ] Adet Passenger Komutları Yüklenecek!`);
    files.forEach(file => {
        let referans = require(`./passenger/Komutlar/${file}`);
  ...
  fs.readdir("./passenger/Etkinlikler/", (err, files) => {
    if (err) return console.myTime(err);
    files.forEach(fileName => {
      let referans = require("./passenger/Etkinlikler/" + fileName);
        referans.onLoad(client);
        client.on(referans.Etkinlik, referans.onRequest);
      console.log(
        `[~ Passenger ~ ] (${fileName}) isimli etkinlik yüklendi.`
      );
    });
  });
...
/*client.on('ready', ()=>{
client.channels.cache.get('825690349392625674').join()
})*/
...
/*client.on("guildMemberAdd", async member => {
          ...
          let passengerkalkmazban = qDb.get(`akb_${member.guild.id}`)
          if(passengerkalkmazban && passengerkalkmazban.some(id => `k${member.user.id}` === id)) return;
          let passenger = client.veri;
          let passengerveri = db.get("ayar") || {};
          ...
          if (passengerveri.yasakTaglar && !passengerveri.yasakTaglar.some(tag => member.user.username.includes(tag)) && yasakTaglilar.some(x => x.includes(member.id))) await cezaDb.set('yasakTaglilar', yasakTaglilar.filter(x => !x.includes(member.id)));
           if (passengerveri.yasakTaglar && passengerveri.yasakTaglar.some(tag => member.user.username.includes(tag))) {
           if(passenger.Roller.yasakliTagRolu) member.roles.set([passenger.Roller.yasakliTagRolu]).catch();
      if (!yasakTaglilar.some(id => id.includes(member.id))) cezaDb.push('yasakTaglilar', `y${member.id}`);
      member.send(`**${passenger.Tag} ${passenger.sunucuIsmi}** Yasak tag'da bulunduğunuz için otomatik olarak cezalıya atıldınız tagı çıkartana kadar cezalıda kalmaya devam ediceksin.`).catch();
    } else if (guvenili) {
            if(passenger.Roller.supheliRolu) member.roles.set([passenger.Roller.supheliRolu]).catch();
            if(passenger.Kanallar.supheliLogKanali && member.guild.channels.cache.has(passenger.Kanallar.supheliLogKanali)) return;
          } else if(passenger.kayıtRolleri.kayıtsızRolleri) member.roles.add(passenger.kayıtRolleri.kayıtsızRolleri).catch();
          if(sürelisusturma.some(x => x.id === member.id) || kalicisusturulma.some(x => x.includes(member.id))) member.roles.add(passenger.Roller.muteRolu).catch();
          if(sessusturulma.some(x => x.id === member.id) && member.voice.channel) member.voice.setMute(true).catch();
          if(passenger.IkinciTag) member.setNickname(`${passenger.IkinciTag} İsim | Yaş`).catch();
          else if(passenger.Tag) member.setNickname(`${passenger.Tag} İsim | Yaş`).catch();
          ...
          passengerhook.send(`
  ${client.emojis.cache.get(passenger.Emojiler.hosgeldinGif1)} ${passenger.sunucuUfakIsim} Sunucusuna Hoşgeldin!\n
  ${client.emojis.cache.get(passenger.Emojiler.hosgeldinGif2)} ${member} (\`${member.id}\`) hesabın __${tarih}__ tarihinde ${client.gecmisTarihHesaplama(member.user.createdAt)} oluşturulmuş.\n
  ${client.emojis.cache.get(passenger.Emojiler.hosgeldinGif3)} Ailemiz seninle birlikte **${member.guild.memberCount}** kişiye ulaştı! tagımızı alarak bizlere destek olabilirsin, <@&${passenger.kayıtRolleri.kayıtYapanRoller}> rolüne sahip yetkililer senin ile ilgilenecektir.\n
  ${client.emojis.cache.get(passenger.Emojiler.hosgeldinGif4)} Sunucu kurallarımız <#${passenger.Kanallar.kurallarKanal}> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.\n
  `); */
...
client.login(client.sistem.a_Token).catch(err => console.error("[~ Passenger ~] Discord API Botun tokenini doğrulayamadı."));
