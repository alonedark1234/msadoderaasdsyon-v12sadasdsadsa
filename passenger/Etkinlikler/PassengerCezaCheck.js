...
module.exports = {
  ...
  function yasakKontrolEt() {
    ...
    for (let kisi of kalıcıcezalılar) {
        ...
        if (uye && !uye.roles.cache.has(passenger.Roller.jailRolu)) uye.roles.set(uye.roles.cache.has(passenger.Roller.boosterRolu) ? [passenger.Roller.boosterRolu, passenger.Roller.jailRolu] : [passenger.Roller.jailRolu]).catch();
      };
            ...
            for (let ceza of sürelisusturma) {
              let uye = client.guilds.cache.get(sid).members.cache.get(ceza.id);
                 if (Date.now() >= ceza.kalkmaZamani) {
                     ...
                     if (uye && uye.roles.cache.has(passenger.Roller.muteRolu)) uye.roles.remove(passenger.Roller.muteRolu).catch();
                     kullaniciverisi.set(`ceza.${ceza.No}.BitisZaman`, Date.now())
                } else {
                  if (uye && !uye.roles.cache.has(passenger.Roller.muteRolu)) uye.roles.add(passenger.Roller.muteRolu).catch();
                };
            };
