...
module.exports = {
    Isim: "kick",
    Komut: ["at"],
    Kullanim: "at @Passenger/ID <sebep>",
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if (!victim || !reason) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}kick @Passenger/ID sebep\``).then(x => x.delete({timeout: 5000}));
    ...
    message.react("✅")
    if(passenger.Kanallar.banLogKanali && client.channels.cache.has(passenger.Kanallar.banLogKanali)) client.channels.cache.get(passenger.Kanallar.banLogKanali).send(embed.setFooter(client.altbaslik + ` • Ceza Numarası: #${cezano}`).setDescription(`Yetkili: ${message.author} (\`${message.author.id}\`)\nÜye: ${victim.user.tag} (\`${victim.user.id}\`)\nSebep: **${reason}**`));
