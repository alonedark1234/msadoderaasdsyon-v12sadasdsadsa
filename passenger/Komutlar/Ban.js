...
module.exports = {
    Isim: "ban",
    Komut: ["yasakla"],
    Kullanim: "yasakla @Passenger/ID <sebep>",
  ...
  let embed = new MessageEmbed().setColor('0x2f3136').setAuthor(passenger.Tag + " " + passenger.sunucuIsmi, message.guild.iconURL({dynamic: true, size: 2048}))
  let cezano = cezaNoDb.get(`cezano.${client.sistem.a_SunucuID}`) + 1;
  if(!passenger.Roller.banHammer) return message.channel.send("Sistemsel hata: Rol bulunamadı veya rol bilgileri girilemedi.").then(x => x.delete({timeout: 5000}));
  if(!passenger.Roller.banHammer.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(`Hata: Bu komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`).then(x => x.delete({timeout: 5000}));
  ...
  if (args[0] && (args[0].includes('bilgi') || args[0].includes('info'))) {
    if(!args[1] || isNaN(args[1])) return message.channel.send(`Hata: Lütfen geçerli yasaklanmış üye Id girin!  __Örn:__  \`${client.sistem.a_Prefix}ban bilgi @Passenger/ID\``).then(x => x.delete({timeout: 5000}));;
  ...
  if (!reason) return message.channel.send(`Hata: Lütfen bir sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @Passenger/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  let ceza = {
    No: cezano,
    Cezalanan: victim.id,
  ...
  if (!victim) {
    ...
    if(kisi) {
      ...
      if(passenger.Kanallar.banLogKanali && client.channels.cache.has(passenger.Kanallar.banLogKanali)) client.channels.cache.get(passenger.Kanallar.banLogKanali).send(embed.setDescription(`Yasaklayan Yetkili: ${message.author} (\`${message.author.id}\`)\nYasaklanan Üye: ${kisi.tag} (\`${kisi.id}\`)\nSebepi: ${reason}`).setFooter(client.altbaslik + ` • Ceza Numarası: #${cezano}`));
    } else {
      message.channel.send(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @MODLOF/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
    };
    return message.reply(`Hata: Lütfen bir üye veya sebep giriniz!  __Örn:__  \`${client.sistem.a_Prefix}ban @MODLOF/ID <sebep>\``).then(x => x.delete({timeout: 5000}));
  };
