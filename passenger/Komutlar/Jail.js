...
module.exports = {
    ...
    Kullanim: "cezalı @Passenger/ID <süre> <sebep>",
  ...
  onRequest: async function (client, message, args, guild) {
    let jailicon = client.emojis.cache.get(passenger.Emojiler.jailatildi)
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}jail @Passenger/ID <1s/1m/1h/1d> <sebep>\``).then(sil => sil.delete({timeout: 5000}));
  ...
  if(!sure || !ms(sure) || !reason) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}jail @Passenger/ID <1s/1m/1h/1d> <sebep>\``).then(sil => sil.delete({timeout: 5000}));
  ...
  await uye.roles.set(uye.roles.cache.has(passenger.Roller.boosterRolu) ? [passenger.Roller.boosterRolu, passenger.Roller.jailRolu] : [passenger.Roller.jailRolu]).catch();
