...
module.exports = {
  ...
  onRequest: async (client, message, args) => {
    ...
    message.channel.bulkDelete(Number(args[0])).then(msjlar => message.channel.send(`${client.emoji(passenger.Emojiler.Onay)} Başarıyla <#${message.channel.id}> (\`${message.channel.id}\`) adlı kanal da (**${msjlar.size}**) adet mesaj silindi!`).then(x => x.delete({timeout: 5000}))).catch()
