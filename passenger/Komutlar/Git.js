...
const passengerveri = client.veri.kayıtRolleri;
const passenger = client.veri;
module.exports = {
    Isim: "git",
    Komut: ["git", "izinligit"],
    Kullanim: "izinligit @Passenger/ID",
  ...
  onRequest: async function (client, message, args, guild) {
   ...
   message.channel.send(`${uye}`, {embed: embed.setFooter(client.altbaslik).setAuthor(uye.displayName, uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${message.author} senin ses kanalına girmek için izin istiyor! Onaylıyor musun?`)}).then(async msj => {
      ...
      msj.delete();
      message.channel.send(embed.setFooter(client.altbaslik).setDescription(`${client.emoji(passenger.Emojiler.Iptal)} __**15** Saniye__ boyunca ${uye} isimli kişiden cevap gelmediği için otomatik olarak iptal edildi!`)).then(x => x.delete({timeout: 5000}));
      };	
        });
      });
      } else {
  if (passenger.Roller.moveHammer(rol => message.member.roles.cache.has(rol)) || message.member.hasPermission('ADMINISTRATOR')) {
      await message.member.voice.setChannel(uye.voice.channelID);
      message.channel.send(embed.setDescription(`${client.emoji(passenger.Emojiler.Onay)} ${message.member} isimli __yetkili__ ${uye} isimli __üyenin odasına__ gitti!`)).then(x => x.delete({timeout: 5000}));
    } else {
      const reactionFilter = (reaction, user) => {
        return ['✅'].includes(reaction.emoji.name) && user.id === uye.id;
      };
