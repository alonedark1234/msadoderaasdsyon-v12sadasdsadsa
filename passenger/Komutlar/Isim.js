...
module.exports = {
    Isim: "isim",
    Komut: ["nick", "i"],
    Kullanim: "isim @Passenger/ID <isim> <yaş>",
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if((!passenger.erkekRolleri && !passenger.kadinRolleri) || !passenger.kayıtYapanRoller) return message.channel.send("Sistemsel hata: Rol bulunamadı veya rol bilgileri girilemedi.").then(sil => sil.delete({timeout: 5000}));
    if(!passenger.kayıtYapanRoller.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(`Hata: Bu komutu kullanabilmek için yeterli yetkiye sahip değilsin.`).then(sil => sil.delete({timeout: 5000}));
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}isim @Passenger/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
    ...
    if(!isim || !yaş) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}isim @Passenger/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
        BelirlenenIsim = `${uye.user.username.includes(passengerveri.Tag) ? passengerveri.Tag : (passengerveri.IkinciTag ? passengerveri.IkinciTag : (passengerveri.Tag || ""))} ${isim} | ${yaş}`;
