...
module.exports = {
    Isim: "çek",
    Komut: ["çek", "izinliçek"],
    Kullanim: "izinliçek @Passenger/ID",
      ...
      if (cevap) {
        uye.voice.setChannel(message.member.voice.channelID);
            ...
            message.channel.send(embed.setFooter(client.altbaslik).setDescription(`${uye}, isimli üye ${message.member} üye tarafından __odasına izin ile çekildi__!`)).then(x => x.delete({timeout: 5000}));
             ...
             msj.delete();
      message.channel.send(embed.setFooter(client.altbaslik).setDescription(`${client.emoji(passenger.Emojiler.Iptal)} __**15** Saniye__ boyunca cevap gelmediği için otomatik olarak iptal edildi!`)).then(x => x.delete({timeout: 5000}));
      };
        });
      });
      } else {
     if (passenger.Roller.moveHammer.some(rol => message.member.roles.cache.has(rol)) || message.member.hasPermission('ADMINISTRATOR'))  {
          await uye.voice.setChannel(message.member.voice.channelID);
          message.channel.send(embed.setDescription(`${client.emoji(passenger.Emojiler.Onay)} ${uye} isimli üye ${message.member} isimli __yetkili tarafından__ çekildi!`)).then(x => x.delete({timeout: 5000}));
        } else {
          const reactionFilter = (reaction, user) => {
            return ['✅'].includes(reaction.emoji.name) && user.id === uye.id;
          };
