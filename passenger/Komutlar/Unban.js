...
module.exports = {
    ...
    Kullanim: "yasakkaldır @Passenger/ID <sebep>",
      ...
      try {
        message.guild.fetchBans().then(yasaklar=> {
           if(yasaklar.size == 0) return message.channel.send(`Hata: Sunucu da hiç yasaklama bulunamadı.`).then(x => x.delete({timeout: 5000}));
            ...
            if(passenger.Kanallar.banLogKanali && client.channels.cache.has(passenger.Kanallar.banLogKanali)) client.channels.cache.get(passenger.Kanallar.banLogKanali).send(embed.setTitle('Bir Yasaklama Kaldırıldı').setDescription(`Yasaklamayı Kaldıran Yetkili: ${message.author} (\`${message.author.id}\`)\nYasaklaması Kaldırılan Üye: ${kisi.tag} (\`${kisi.id}\`)\nSebep: **${reason}**`));
             })
      } catch (err) {
          console.log(err)
      }  
