...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    var rolismi = "VIP"
    var rolid = passengerveri.Roller.vipRolu
    let uye = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if (!passengerveri.Roller.botCommand.some(rol => message.member.roles.cache.has(rol)) && !message.member.hasPermission('MANAGE_ROLES') && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`Hata: Bu komutu kullanabilmek için yeterli yetkiye sahip değilsin!`).then(x => x.delete({timeout: 5000}));
     if (!uye) return message.channel.send(`Hata: ${rolismi} rolü verebilmem için lütfen bir üyeyi etiketle __Örn:__ \`${client.sistem.a_Prefix}${module.exports.Isim} @Passenger/ID\`!`).then(x => x.delete({timeout: 5000}));
